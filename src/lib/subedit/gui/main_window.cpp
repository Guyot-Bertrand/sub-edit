...
public:
    explicit VideoSide(MainWindow& window) : m_window(&window) {}
...
MainWindow::MainWindow(core::FileSystem& files,
    ...
    : QMainWindow(parent),
      ...
      m_videoSide(std::make_unique<VideoSide>(*this)),
      m_video(std::make_unique<VideoPane>(*m_files,
                                          *m_prompts,
                                          *m_videoSide,
                                          *m_table,
                                          *m_split,
                                          std::move(buildPlayer),
                                          std::move(readDeclaredRate),
                                          this)) {
    ...
    // The picture on top — the pane put it there, and its band — the table
    // under it, and the line between them draggable — which is the one thing a
    // fixed layout could not give.
    QSplitter* split = m_split;
    split->addWidget(m_table);
    connect(m_video->invite(), &QAbstractButton::clicked, this, &MainWindow::selectVideo);
...
void MainWindow::openOn(core::Project project, std::span<const core::Diagnostic> diagnostics) {
    ...
    connect(page->tableSelection.get(), &QItemSelectionModel::selectionChanged, this, [this] {
        m_video->placeAtSelection(*m_page);
    });
...
void MainWindow::refreshForPage() {
    ...
    // Reads `m_page->associated` against the page the player plays for, not only against
...
void MainWindow::selectVideo() {
    if (m_video->choose(*m_page))
        refreshVideo();
}

void MainWindow::proposeVideoBeside() {
    m_video->proposeBeside(*m_page);
    refreshVideo();
}
...
void MainWindow::refreshVideo() {
    ...
    // `ffprobe`, once per film — so painting the status bar before it is
