...
| casse, `ß` en tête | `str.capitalize` rend `Ss` — vérifié en exécutant la méthode seule | `sentenced` laisse `ß` — une lettre qui s'allonge en capitale garde sa forme. **Écart**, et le commentaire qui le prêtait à Gaupol est corrigé (#401) |
